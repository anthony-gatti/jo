...
if (!Array.prototype.indexOf)
{
  Array.prototype.indexOf = function(elt /*, from*/)
  {
	...
	var defaults = {
		...
		// Enable scroll_to_explor_click link
		scroll_to_explor_click: true,
	...
	Plugin.prototype = {
		...
		/**
		 * scroll_to_explor link.
		 *
		 * @returns {Plugin}
		 */
		scroll_to_explor_click: function () {
			var self = this;
			$(".scroll_to_explore").click(function(event) {
             event.preventDefault();
            self.customScrollTo(1);
        });
			return this;
		},
		...
		customScrollTo: function (index, noAnimation) {
			...
			if (index != null && index >= 0 && index < this._sections) {
				...
				var theNav = this._$nav
				if (theNav) {
					theNav._$menuitems.removeClass(this.options.active);
					$('a[href="#' + this._sectionIdentifiers[index] + '"]', theNav).addClass(this.options.active);
				}
		...
		init: function () {
			...
			if(hash_value == "project_thumbs"){
		 	self.customScrollTo(2,0);
		 	console.log("custom scroll to thumb list");
			}
	...
	/**
	* Scroll to thumb list if hashtag if found
	*/
